...
import EmployeeWorkHoursComparisonChart from './components/EmployeeWorkHoursComparisonChart'
...
export default function DashboardPage() {
  ...
  const { overview, comparison, ranking, exceptions, initialize, refresh, retry, dispose } = useDashboardStore(useShallow((s)=>({overview:s.overview,comparison:s.comparison,ranking:s.ranking,exceptions:s.exceptions,initialize:s.initialize,refresh:s.refresh,retry:s.retry,dispose:s.dispose})))
  ...
  return (
    <div className="flex min-w-0 flex-col gap-5" data-testid="attendance-dashboard">
      ...
      <section className="rounded-xl border border-bd bg-surface p-4 shadow-sm"><h2 className="mb-4 text-lg font-semibold">{t('nav.dashboard.chart.title')}</h2><DashboardPanelState status={comparison.status} onRetry={()=>void retry('comparison')}>{comparison.data&&<EmployeeWorkHoursComparisonChart data={comparison.data}/>}</DashboardPanelState></section>
