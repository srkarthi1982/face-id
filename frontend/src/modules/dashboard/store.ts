...
import type { SuccessResponseDashboardOverview, SuccessResponseDashboardTrend, SuccessResponseEmployeeWorkHoursRanking, SuccessResponseListAttendanceExceptionItem, SuccessResponseListDashboardOrganizationOption } from '../../api/generated'
import type { DashboardFilters, DashboardOrganizationOption, DashboardOverview, DashboardTrend, EmployeeWorkHoursRanking, ExceptionResult, PanelState } from './types'

type Section = 'organizations' | 'overview' | 'trend' | 'ranking' | 'exceptions'
type FilterError = 'dateRange' | null
type SdkResult<T> = { data?: T; error?: unknown; response: Response }

const analyticsSections: Section[] = ['overview', 'trend', 'ranking', 'exceptions']
...
const controllers: Record<Section, AbortController | null> = { organizations: null, overview: null, trend: null, ranking: null, exceptions: null }
const requestIds: Record<Section, number> = { organizations: 0, overview: 0, trend: 0, ranking: 0, exceptions: 0 }
...
export const useDashboardStore = create<State>((set, get) => {
  ...
  const loadSection = async (section: Section) => {
    ...
    if (section === 'trend') return run<SuccessResponseDashboardTrend, DashboardTrend>(section, (signal) => fetchTrend({ query: { ...base, granularity: filters.granularity }, signal }), (data) => data.data, (data) => data.points.length === 0)
  ...
  return {
    ...
    organizations: panel(), overview: panel(), trend: panel(), ranking: panel(), exceptions: panel(),
    ...
    setGranularity: async (value) => { set((state) => ({ draft: { ...state.draft, granularity: value }, applied: { ...state.applied, granularity: value } })); await loadSection('trend') },
