...
import type { SuccessResponseDashboardOverview, SuccessResponseDashboardTrend, SuccessResponseEmployeeWorkHoursRanking, SuccessResponseListAttendanceExceptionItem } from '../../api/generated'
...
type Section = 'overview' | 'trend' | 'ranking' | 'exceptions'
type FilterError = 'dateRange' | null
type SdkResult<T> = { data?: T; error?: unknown; response: Response }

const sections: Section[] = ['overview', 'trend', 'ranking', 'exceptions']
...
const controllers: Record<Section, AbortController | null> = { overview: null, trend: null, ranking: null, exceptions: null }
const requestIds: Record<Section, number> = { overview: 0, trend: 0, ranking: 0, exceptions: 0 }
...
export const useDashboardStore = create<State>((set, get) => {
  ...
  return {
    ...
    overview: panel(), trend: panel(), ranking: panel(), exceptions: panel(),
    setDraft: (patch) => set((state) => ({ draft: { ...state.draft, ...patch }, filterError: null })),
    loadAll: async () => { await Promise.allSettled(sections.map(loadSection)) },
    apply: async () => {
