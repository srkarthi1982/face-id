...
export default function DashboardFilters() {
  const { t } = useI18n()
  const { draft, filterError, departments, setDraft, apply, reset } = useDashboardStore(useShallow((s) => ({
    draft: s.draft, filterError: s.filterError, departments: s.departments, setDraft: s.setDraft, apply: s.apply, reset: s.reset,
  })))
  const departmentOptions = departments.data ?? []
  const selectedDepartment = draft.department_id ? String(draft.department_id) : ''
  const showSelectedFallback = Boolean(selectedDepartment && !departmentOptions.some((option) => String(option.department_id) === selectedDepartment))
  return (
    <section className="grid gap-4 rounded-xl border border-bd bg-surface p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1.2fr_auto]" aria-label={t('nav.dashboard.filters.title')}>
      ...
      <label className="flex flex-col gap-1 text-sm text-secondary"><span>{t('nav.dashboard.filters.department')}</span><select data-testid="dashboard-department" value={selectedDepartment} onChange={(event) => setDraft({ department_id: event.target.value ? Number(event.target.value) : undefined })} className="min-h-11 rounded-lg border border-bd bg-surface-2 px-3 text-primary focus:outline-none focus:ring-2 focus:ring-accent"><option value="">{t('nav.dashboard.filters.allDepartments')}</option>{showSelectedFallback && <option value={selectedDepartment}>{selectedDepartment}</option>}{departmentOptions.map((option) => <option key={option.department_id} value={option.department_id}>{option.department_name}</option>)}</select>{departments.status === 'loading' && <span role="status" className="text-xs text-muted">{t('nav.dashboard.filters.departmentsLoading')}</span>}{(departments.status === 'error' || departments.status === 'unavailable') && <span role="alert" className="text-xs text-[var(--danger)]">{t('nav.dashboard.filters.departmentsUnavailable')}</span>}</label>
