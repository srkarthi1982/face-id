...
export default function DashboardFilters() {
  ...
  const selectedOrganization = draft.org_id?.trim() || ''
  const showSelectedFallback = Boolean(selectedOrganization && !organizationOptions.some((option) => option.org_id === selectedOrganization))
  return (
    <section className="grid gap-4 rounded-xl border border-bd bg-surface p-4 shadow-sm sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1.2fr_auto]" aria-label={t('nav.dashboard.filters.title')}>
      ...
      <label className="flex flex-col gap-1 text-sm text-secondary"><span>{t('nav.dashboard.filters.organization')}</span><select data-testid="dashboard-org" value={selectedOrganization} onChange={(event) => setDraft({ org_id: event.target.value || undefined })} className="min-h-11 rounded-lg border border-bd bg-surface-2 px-3 text-primary focus:outline-none focus:ring-2 focus:ring-accent"><option value="">{t('nav.dashboard.filters.allOrganizations')}</option>{showSelectedFallback && <option value={selectedOrganization}>{selectedOrganization}</option>}{organizationOptions.map((option) => <option key={option.org_id} value={option.org_id}>{option.org_id}</option>)}</select>{organizations.status === 'loading' && <span role="status" className="text-xs text-muted">{t('nav.dashboard.filters.organizationsLoading')}</span>}{(organizations.status === 'error' || organizations.status === 'unavailable') && <span role="alert" className="text-xs text-[var(--danger)]">{t('nav.dashboard.filters.organizationsUnavailable')}</span>}</label>
