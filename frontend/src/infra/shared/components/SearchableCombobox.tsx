...
export interface SearchableComboboxProps {
    ...
    placeholder?: string;
    onChange?: (selected: { id: string | number; value: string; meta?: any } | null) => void;
...
export const SearchableCombobox: React.FC<SearchableComboboxProps> = ({
    ...
    const handleSelect = useCallback((option: SelectOption) => {
        ...
        setIsOpen(false);
        setHighlightedIndex(-1);
        onChange?.({ id: option.id, value: option.value, meta: option.meta });
    }, [onChange]);
    ...
    const handleAddNew = useCallback(() => {
        ...
        if (newValue) {
            ...
            setIsOpen(false);
            setHighlightedIndex(-1);
            // Keep the query value so user can see what they typed
                ...
                >
                    {groupedOptions ? (
                        ...
                        Object.entries(groupedOptions).map(([group, groupOpts], groupIndex) => {
                            const offset = Object.keys(groupedOptions)
                                .slice(0, groupIndex)
                                .reduce((sum, key) => sum + groupedOptions[key].length + 1, 0);
                            
                            return (
                                <li key={group} role="group">
                                    {renderGroupHeader ? (
                                        renderGroupHeader(group, groupOpts.length)
                                    ) : (
                                    ...
                                    {groupOpts.map((opt, index) => {
                                        ...
                                        return (
                                            <li
                                                ...
                                                className={`pl-8 pr-4 py-2.5 text-sm cursor-pointer transition-colors ${globalIndex === highlightedIndex
                    ...
                    {filteredOptions.length === 0 && !showAddNew && !groupedOptions && (
