...
│   ├── user-profile/        (UserProfileInfoPage lives here — no manifest; also used at root '/')
│   └── design-system/       (DesignSystemPage — no manifest; hidden static route in App.tsx, gated by 'admin:full')
├── dynamic-forms/           (path 'dynamic-forms', permissions ['*hidden*'] → matches no user, module hidden, order 20, page DynamicFormsPage)
├── dashboard-scheduling/    (path '/dashboard-scheduling', permissions [] → public-with-auth, order 30)
│   ├── dashboard/           (DashboardPage + store — no manifest, not routed)
│   ├── schedule-management/ (path 'schedule-management/*', page ScheduleManagementRoutes, permissions ['schedule_entry:read'], order 10)
│   └── progress-tracker/    (permissions ['progress_tracker:*'], order 40)
├── course-management/       (path '/course-management', permissions ['course_master:*','course:*','library:*','student:*','teacher:*'], order 40)
│   ├── course-builder/      (path 'course-builder/*', page CourseBuilderRoutes, permissions ['course_master:*'], order 10)
│   ├── course-selection/    (path 'course-selection/*', page CourseSelectionRoutes, permissions ['course:*'], order 15)
│   ├── enrolled-students/   (permissions ['student:*'])
│   ├── library/             (page LibraryPanelPage, permissions [])
│   ├── my-courses/          (permissions ['teacher:*','admin:full'])
│   └── _shared/ components/ course-info/ currencies-certificate/ evaluation/ material/ survey/
│                            (no manifests — sub-pages & helpers consumed by the wildcard routes above)
├── assignment-assessment/   (path '/assignment-assessment', permissions ['quiz:*'], order 50)
│   └── quiz-bank/           (permissions ['quiz:*'], order 10)
├── grading-attendance/      (path '/grading-attendance', permissions ['student:*','teacher:*','attendance:*'], order 60)
│   ├── class-attendance/    (permissions ['student:*','teacher:*','attendance:*'], order 21)
│   ├── daily-attendance/    (permissions ['student:*','teacher:*','attendance:*'], order 22)
│   ├── formnew/             (page FormDefaultPage, permissions ['form:*'], order 60)
│   ├── surveynew/           (page SurveyDefaultPage, permissions ['survey:*'], order 60)
│   ├── grading/             (path 'grading/*', page GradingRoutes, permissions ['grade:*'], order 70)
│   └── attendance/ components/ survey/   (no manifests — support/legacy code)
├── communication-reporting/ (path '/communication-reporting', permissions ['teacher:*','student:*'], order 70)
│   ├── notification/        (order 10)
│   ├── requests/            (order 15)
│   ├── it-support/          (permissions ['ticket:create','ticket:respond','ticket:view_all'], order 17)
│   ├── chat/                (order 20)
│   ├── faq/                 (permissions ['faq:read'], order 25)
│   ├── file-sharing/        (permissions ['file:read'], order 30)
│   └── virtual-classroom/   (permissions ['class_session:*'], order 40; detail routes ':id' and ':id/live' are static in App.tsx)
├── external-link/           (path '/external-link', permissions ['teacher:*','student:*'], order 80, page AppsPage)
│   └── apps/                (AppsPage lives here — no feature manifest)
└── settings/                (path '/settings', public, order 90, pinBottom)
    ├── account/             (order 5)
    ├── access-management/   (path 'access-management/*', page AccessManagementRoutes, permissions ['admin:*'], order 10)
    │   └── access-control/ permissions/ roles/   (sub-pages routed internally by AccessManagementRoutes — NOT auto-discovered)
    ├── rag-admin/           (path 'rag-admin/*', page RagAdminRoutes, permissions ['admin:full'], order 15)
    │   └── overview/ ingestion/ chunks/ playground/   (sub-pages routed internally)
    ├── users-management/    (path 'users', page UsersPage, permissions ['admin:full'], order 20)
    ├── appearance/          (order 25)
    ├── language/            (order 30)
    ├── audit-log/           (permissions ['audit:read'], order 30)
    └── usage-analytics/     (permissions ['analytics:read'], order 31)
```
...
### Wildcard feature paths (nested sub-routes)

Auto-discovery only globs **two levels deep** (`modules/*/manifest.ts` and `modules/*/*/manifest.ts`). A feature that needs its own nested pages declares a wildcard path and owns the routing below it:

```ts
// settings/access-management/manifest.ts
path: 'access-management/*',      // trailing /* — React Router forwards everything below
page: AccessManagementRoutes,     // a component that renders its own <Routes>
```

The `page` component renders an internal `<Routes>` mapping the sub-paths to sub-page components (see `settings/access-management/AccessManagementRoutes.tsx`, `settings/rag-admin/RagAdminRoutes.tsx`, `course-management/course-builder/`, `grading-attendance/grading/`, `dashboard-scheduling/schedule-management/`). Any `manifest.ts` placed three levels deep is **ignored by the menu generator** — sub-pages get no automatic sidebar/submenu entry and must be wired inside the internal routes component.
...
- `/` — `UserProfileInfoPage` (gated by `teacher:read` / `student:read` / `admin:full`)
- `/design-system` — `DesignSystemPage` (admin-only)
- `/login`, `/register`, `/unauthorized`
- `/communication-reporting/virtual-classroom/:id` and `.../:id/live` — virtual-classroom detail/live views (param routes can't come from manifests)
- `/common/dashboard` — legacy redirect to `/`
...
**New feature inside an existing module** — most common. Pick the module that matches the feature's purpose (`profile-general-info`, `dynamic-forms`, `dashboard-scheduling`, `course-management`, `assignment-assessment`, `grading-attendance`, `communication-reporting`, `external-link`, `settings`). Admin tooling (`access-management`, `users-management`, `rag-admin`, `audit-log`, `usage-analytics`) lives as **features under `settings/`**, not as top-level modules.
...
Use when you need multiple list states, custom fetch logic, extra actions, or state that doesn't fit `createCrudStore`. Reference: `src/modules/settings/users-management/pages/store.ts`.
